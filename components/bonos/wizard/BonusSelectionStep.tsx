...
export function BonusSelectionStep() {
  ...
  // Budget calculation based on split (default 60/40)
  ...
  const presupuestoBonos = useMemo(
    () => totalNomina * (pctBonos / 100),
    [totalNomina, pctBonos]
  )

  // Calculate total assigned to 40% budget (only bonuses whose category counts)
  const totalAsignado40 = useMemo(() => {
    ...
    for (const [tipo, cfg] of Object.entries(localConfig)) {
      ...
      if (!meta) continue
      if (!CATEGORIES_THAT_COUNT.has(meta.categoria)) continue
      const resolved = resolveBonus(cfg, salarioPromedio)
      sum += resolved * totalEmpleados
    }
    return sum
  }, [localConfig, salarioPromedio, totalEmpleados])
  ...
  const puedeAvanzar = useMemo(() => {
    ...
    return true
  }, [tiposSeleccionados, localConfig, excedido])
      ...
      {totalNomina > 0 && (
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
            ...
            <div className="hidden sm:block w-px h-4 bg-gray-300" />
            <div className="flex items-center gap-2">
              <span className="text-gray-500">Presupuesto bonos ({pctBonos}%):</span>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3">
                  ...
                  {section.countsToward40 ? (
                    <span className="px-2.5 py-0.5 bg-green-100 text-green-700 text-xs rounded-full font-medium whitespace-nowrap">
                      Suma al {pctBonos}%
                    </span>
                  ) : (
                    <span className="px-2.5 py-0.5 bg-gray-100 text-gray-500 text-xs rounded-full font-medium whitespace-nowrap">
                      No suma al {pctBonos}%
                    </span>
                  )}
                </div>
                    ...
                    {isSelected && cfg && (
                      <div className="px-4 pb-4 pt-1 border-t border-red-100">
                        <div className="space-y-3">
                          ...
                          {cfg.valor > 0 && excedido && meta && CATEGORIES_THAT_COUNT.has(meta.categoria) && (
                            <p className="text-xs text-red-600 font-medium mt-1">
                              Excede el presupuesto del {pctBonos}%
                            </p>
                          )}
                        </div>
                      </div>
